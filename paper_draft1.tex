...
We need to explain how to preserve conservation of flux at grid interfaces, Fig \ref{fig21}. There are essentially three cases to consider: coarse cell to coarse cell, fine cell to fine cell, and coarse cell to fine cell. Let $F_i$ be the flux at the left edge of coarse cell $i$. This is different than what we typically wrote in class, $F_{i-\frac{1}{2}}$, for convenience. Thus, updating between coarse grid cells is simply:
\begin{equation}
...
The correction step extends to an arbitrary hyperbolic system where we use a Riemann solver to obtain $A^{-}\Delta q$ and  $A^{+}\Delta q$ from the two states $\hat{q}_{m+1}^{0}$ and $q_{j}^{0}$.  This step is made in each of the $R$ time steps on the refined grid within the single coarse-grid step, where $R$ is the refinement ratio ($2X$ in our examples so far). \\
...
A similar modification must be made in each of the R time steps on the refined grid within the single coarse-grid step, where R is again the refinement ratio. The fix-up algorithm thus takes the form?for N = 0, 1, . . . , R ? 1 dosolve the Riemann problem with data qö and qto compute A?Æq and A+Æq, update q1 := q1 + kö (A?Æq + A+Æq).jjhFor the case of a conservation law, this will restore conservation, and, in fact, agreeswith the flux function modification (2.4) if numerical fluxes are defined by (3.9).Note that, in view of (3.8), in the conservation law case the coarse-grid value qj1 isN0simply updated by the entire flux difference f (qö ) ? f (q ), and we wouldnÕt needm+1 jto actually solve the Riemann problem. However, implementing it as presented abovegives a uniform and general formulation.
